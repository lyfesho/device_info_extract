...
def mac_gene_priority_feature(json_root_arr, json_feature_arr):
    for mac_obj in json_root_arr:
        mac_feature_obj = find_mac_obj(json_feature_arr, mac_obj['mac'])
        #mac prefix
        mac_addr_obj['vendor'] = mac_obj['feature_label']['vendor']
        #dhcp
        if 'dhcp_hostname' in mac_obj['feature_label']:
            dhcp_hostname_obj['dhcp_hostname'] = mac_obj['feature_label']['dhcp_hostname']
        if 'dhcp_userclass' in mac_obj['feature_label']:
            dhcp_userclass_obj['dhcp_userclass'] = mac_obj['feature_label']['dhcp_userclass']
        if 'dhcp_vendorclass' in mac_obj['feature_label']:
            dhcp_vendorclass_obj['dhcp_vendorclass'] = mac_obj['feature_label']['dhcp_vendorclass']
        #dhcpv6
        if 'dhcpv6' in mac_obj['feature_label']:
            dhcpv6_obj['dhcpv6'] = mac_obj['feature_label']['dhcpv6']
        #mdns
        if 'mdns_name' in mac_obj['feature_label']:
            mdns_name_obj['mdns_name'] = mac_obj['feature_label']['mdns_name']
        if 'mdns_model' in mac_obj['feature_label']:
            mdns_model_obj['mdns_model'] = mac_obj['feature_label']['mdns_model']
        #ssdp
...
for root, dirs, files in os.walk(pkt_dir, False):
    if files:
        for line in files:
            if '.json' not in line:
                #file_cnt = file_cnt + 1
                file_cnt = line.split('-')[0]
		        #if (int(file_cnt) <= 34):
		        #    continue
                if (int(file_cnt) <= 666):
                    continue
                filepath = os.path.join(root, line)
                file_name = filepath
            
                csv_file = './support/knowledge.csv'
                output_json_name = 'train_data' + str(file_cnt) + '.json'
                output_csv_name = 'result' + str(file_cnt) +'.csv'
                ...
                #generate priority feature
                #mac_gene_priority_feature(json_root_arr, json_feature_arr)   
